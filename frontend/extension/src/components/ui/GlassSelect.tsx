...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      "flex h-10 w-full items-center justify-between rounded-glass px-3 py-2 text-sm",
      "glass-medium placeholder:text-muted-foreground",
...
>(({ className, children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        "relative z-50 max-h-96 min-w-[8rem] overflow-hidden rounded-glass shadow-glass-medium",
        "glass-heavy animate-slide-in",
        position === "popper" &&
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ref={ref}
    className={cn(
      "relative flex w-full cursor-default select-none items-center rounded-glass-sm py-1.5 pl-8 pr-2 text-sm outline-none",
      "focus:glass-light focus:text-accent-foreground",
